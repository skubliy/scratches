...
public:
    ...
    shm_obj( const char* mid, const char* suffix, int _flags=O_RDONLY) :
        pmmap(nullptr), fd(-1), fmode(_flags) {

        // crating shmem file name
        static const char* prefix =  "_shmo-";
        ...
        //TODO need to be sure the O_CREATE will be only onse
    ...
    virtual ~shm_obj() {
        ...
        if ( fd > 0 ) {
            ::close( fd );
            fd = 0;
            LOG( LOG_DEBUG, "zzz some thing strange here Shared memory objects file successfully closed");
        }
