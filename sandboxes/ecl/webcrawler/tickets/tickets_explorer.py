...
# encoding: utf-8
'''
exploring flight tickets for best price
...
airports = {'YVR': 'Vancouver,%20BC,%20Canada%20(YVR-All%20Airports)'
           ,'SEA': 'Seattle,%20WA,%20United%20States%20(SEA-Seattle%20-%20Tacoma%20Intl.)'
           }
#https://www.expedia.ca/Flights-Search?trip=roundtrip
#&leg1=from:Vancouver,%20BC,%20Canada%20(YVR-All%20Airports),to:MIA,departure:24/02/2016TANYT
#&leg2=from:MIA,to:Vancouver,%20BC,%20Canada%20(YVR-All%20Airports),departure:03/03/2016TANYT
#&passengers=children:0,adults:1,seniors:0,infantinlap:Y&mode=search
sites = {'expedia' : "https://www.expedia.ca/Flights-Search?trip=roundtrip\
...
class url_holder():
    ...
    def get_url(self, apfrom, apto, dsate, rdate, nad = 2, nch = 1):
        self.last_url = self.turl%(apfrom, apto, dsate, rdate, nad, nch)
...
Accept-Language: en-GB,en-US;q=0.8,en;q=0.6"
    ...
    def do_request(self, url = None):
        ...
        print "##########"
        # *** urlib2 ***
        request = urllib2.Request(url.get, urllib.urlencode({'name' : 'SKub'}), self.hdrs)
...
def do_compile(urlholder):
    n = 0
    sleep_time = 1
    while True:
        usr=client("usr-%d"%n)
        n += 1
        usr.do_request(urlholder.get_url())
...
def do_request(usr = "usr-new", urlholder = None):
    if "usr_new" in usr:
