...
def print_memory_usage(sorted_cmds, shareds, count, total):
    ...
    for cmd in sorted_cmds:
        xother = ("%s"%((cmd[1]-shareds[cmd[0]])*1024)).split('.')[0]
        xshare = ("%s"%(shareds[cmd[0]]*1024)).split('.')[0]
        xall  = ("%s"%(cmd[1]*1024)).split('.')[0]
        sys.stdout.write("mob-other : %s\nmob-shared : %s\nmob-all-mem : %s\n"%(xother, xshare, xall))
                        #%((human(cmd[1]-shareds[cmd[0]]), human(shareds[cmd[0]]), human(cmd[1]))))
