...
public:
    ...
    public:
        ...
        TD value_by_index(uint32_t idx) const {
            ...
            uint32_t version_before = rarr.rvers[idx]; //it not must be atomic copy assignment
            ...
            uint32_t version_after = rarr.rvers[idx]; // it must be atomic copy assignment
    ...
    public:
        ...
        void invalidate_array(void){
            ...
            for ( uint32_t idx = 0; idx < MAX_IDX; idx++ ) {
                ...
                if (_rarr.rvals[idx] != this->zero()){
                    // _rarr.rvers[idx]++;
                    fetchAndAdd<TV>(&_rarr.rvers[idx], 1);
                    _rarr.rvals[idx] = this->zero(); //FIXME need function to invalidate
                    // _rarr.rvers[idx]++;
                    fetchAndAdd<TV>(&_rarr.rvers[idx], 1);
                }
            }
        ...
        void set_val_by_idx_for_test(uint32_t idx, TD val){
            this->rarr.rflgs[idx] = false;
            //++this->rarr.rvers[idx];
            //fetchAndAdd<TV>(&this->rarr.rvers[idx], 1);
            __sync_fetch_and_add( &this->rarr.rvers[idx], 1 );
            this->rarr.rvals[idx] = val; //it is must be copy assigned.
            //++this->rarr.rvers[idx];
            //fetchAndAdd<TV>(&this->rarr.rvers[idx], 1);
            __sync_fetch_and_add( &this->rarr.rvers[idx], 1 );
            ...
            for ( new_vals_list_iter itr = nwl.begin(); itr != nwl.end();) {
                ...
                for (idx=0; idx < MAX_IDX; idx++) {
                    if (( _rarr.rvals[idx] == this->zero() ) || (_rarr.rflgs[idx] == false)) {
                        ...
                        // _rarr.rvers[idx]++;
                         fetchAndAdd<TV>(&_rarr.rvers[idx], 1);
                        _rarr.rvals[idx] = *itr; //it is must be copy assigned.
                        // _rarr.rvers[idx]++;
                         fetchAndAdd<TV>(&_rarr.rvers[idx], 1);
